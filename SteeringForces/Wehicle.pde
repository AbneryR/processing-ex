class Wehicle{
  ...
  void seek(PVector target){ //Algoritmo de fuerza de busqueda
    PVector desired = PVector.sub(target,loc); //Calcular la velocidad deseada
    desired.normalize();
  ...
  void flee(PVector target){ //Algoritmo de fuerza de busqueda
    PVector desired = PVector.sub(target,loc); //Calcular la velocidad deseada
    desired.normalize();
    desired.mult(-maxspeed); //Calculando la velocidad deseada al objetivo a maxima vlocidad
    PVector steer = PVector.sub(desired,vel);//Formula de Reynold's :Steering force es igual a velocidad deseada - velocidad actual
